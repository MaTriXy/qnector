...
//				System.out.println(parts.get(i).getName() + " is from "
//						+ parts.get(i).getDeviceSet() + " and is "
//						+ parts.get(i).getDevice() + " which is device type "
//						+ parts.get(i).getPackageType());
			...
			for (int i = 0; i < wires.size(); i++) {
				System.out.println(wires.get(i).side() + "="
						+ wires.get(i).rowS() + "=" + wires.get(i).rowE());
			}
	...
	private static ArrayList<Part> getPartsList(Document doc) {
		...
		for (int i = 0; i < partsList.getLength(); i++) {
			...
			String deviceSet = ((Element) partsList.item(i))
					.getAttribute("deviceset");
			String device = ((Element) partsList.item(i))
					.getAttribute("device");
	...
	private static void populateNetAndPins(Document doc, ArrayList<Part> parts,
			ArrayList<Net> netList) {
		...
		for (int netIndex = 0; netIndex < nl2.getLength(); netIndex++) {
			...
			netList.add(newNet);
			NodeList pinrefs = ((Element) nl2.item(netIndex))
					.getElementsByTagName("pinref");
			for (int j = 0; j < pinrefs.getLength(); j++) {
				String targetName = ((Element) pinrefs.item(j))
						.getAttribute("part");
				for (int partIndex = 0; partIndex < parts.size(); partIndex++) {
					if (targetName.equals(parts.get(partIndex).getName())) {
						...
						int pinNumber;
						try {
							pinNumber = Integer.parseInt(((Element) pinrefs
									.item(j)).getAttribute("pin"));
						} catch (NumberFormatException e) {
							if (((Element) pinrefs.item(j)).getAttribute("pin")
									.equals("+"))
								pinNumber = 1;
							else
								pinNumber = 2;
			...
			if (nDevice.getNodeType() == Node.ELEMENT_NODE) {
				...
				while (it.hasNext()) {
					...
					if (eDevice.getAttribute("name").equals(temp.getDevice())) {
						NodeList connects = eDevice
								.getElementsByTagName("connects");
						int numOfPins = ((Element) connects.item(0))
								.getElementsByTagName("connect").getLength();
	...
	/*
	 * private static ArrayList<Wire> generateWires(ArrayList<Net> netList,
	 * ArrayList<Part> partsList) { ArrayList<Wire> wires = new
	 * ArrayList<Wire>();
	 * 
	 * 
	 * for (int i = 0; i < netList.size(); i++) { Net tempNet = netList.get(i);
	 * Part tempPart = partsList.get(tempNet.getParts().get(0)); //get the first
	 * part listed in the first net int[] rowArr = tempPart.getPinRow(); int[]
	 * sideArr = tempPart.getPinSide();
	 * tempNet.getRows().add(rowArr[tempNet.getPins().get(0)-1]); //add the row
	 * info for the first pin in the net list
	 * tempNet.getSides().add(sideArr[tempNet.getPins().get(0)-1]); for (int j =
	 * 1; j < tempNet.getParts().size(); j++) { tempPart =
	 * partsList.get(tempNet.getParts().get(j)); rowArr = tempPart.getPinRow();
	 * int row = rowArr[tempNet.getPins().get(j)-1]; sideArr =
	 * tempPart.getPinSide(); int side = rowArr[tempNet.getPins().get(j)-1];
	 * 
	 * int ret = rowInNetWrongSide(tempNet, row, side); if (ret > -1) { int
	 * sideChange = 1; int row1 = tempNet.getRows().get(ret); int row2 = row1;
	 * Wire tempWire = new Wire(sideChange, row1, row2); wires.add(tempWire); }
	 * 
	 * ret = sideInNetWrongRow(tempNet, row, side); if (ret > -1) { int row1 =
	 * tempNet.getPins().get(ret); int row2 = row; Wire tempWire = new Wire(0,
	 * row1, row2); wires.add(tempWire); }
	 * 
	 * if (!alreadyInNet(tempNet, row, side)) { int sideChange = 1; int row1 =
	 * row; int row2 = tempNet.getRows().get(tempNet.getRows().size()-1); Wire
	 * tempWire = new Wire(1, row1, row2); wires.add(tempWire); }
	 * 
	 * tempNet.getRows().add(row); tempNet.getSides().add(side);
	 * 
	 * } } return wires; }
	 */
	private static ArrayList<Wire> generateWires(ArrayList<Net> netList,
		...
		for (int netIndex = 0; netIndex < netList.size(); netIndex++) // for
																		// every
																		// net
		{
			for (int i = 0; i < netList.get(netIndex).getParts().size() - 1; i++) // loop
																					// through
																					// every
																					// pair
																					// of
